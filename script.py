...
def download_audio_and_cover(video_url, output_dir, user):
	...
	if info is None:
		print(f"⚠️ Skipped (info None) : {video_url}")
		update_status(user, "NULL", video_url, "error", error="Impossible de récupérer les infos")
		return

	sanitized = get_sanitized_filename(emtry_info)
...
def download_playlist_audio_and_covers(url, output_dir, user):
	...
	if info is None:
		print(f"❌ Impossible de charger la playlist : {url}")
		return
	...
	if 'entries' in info:
		...
		for entry in entries:
			try:
				with YoutubeDL(info_opts) as ydl2:
					entry_info = ydl2.extract_info(entry['url'], download=False)
				if entry_info is None:
					print(f"⚠️ Skipped (info None) : {entry.get('url')}")
					continue
				name = get_sanitized_filename(entry_info)
				update_status(user, name, entry['url'], "queued", 0)
			except Exception as e:
				print(f"⚠️ Skipped (erreur info) : {e}")
				continue
